...
class AssetOwnerStorage extends EntityStorage<AssetOwner> {
  ...
  public override async createEntity(block: SubstrateBlock, id: string, sbtAccesses: AssetOwnerAccount): Promise<Asset> {
    // will it store ???
    return new AssetOwner(id, sbtAccesses);
  }

  async updateAccess(block: SubstrateBlock, id: string, accountId: string, expiresAt: number, sbtAssetId): Promise<void> {
    const assetOwner = await this.getEntity(block, id);
    const currentSbtAccesses = assetOwner.sbtAccesses ? JSON.parse(assetOwner.sbtAccesses) : [];
    ...
    assetOwner.sbtAccesses = JSON.stringify(updated);
