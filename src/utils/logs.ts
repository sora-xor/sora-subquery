...
export function getLog(
  ...
  attrs: Record<string, LogAttr> = {},
  onlyWithTestLogMode: boolean = false
) {
  ...
  const attrsToString = (attributes: Record<string, LogAttr>): string => {
    return Object.entries(attributes)
      .map(([key, value]) => ' '.repeat(41) + `\x1b[30m${key}: ${value.toString()}\x1b[0m`)
      .join('\n');
  };

  const log =
    (level: 'debug' | 'info' | 'warn' | 'error') => (arg1: Record<string, LogAttr> | string, arg2?: string) => {
